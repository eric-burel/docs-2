...
2. **Mint or exchange a credential for that user.** Inside the [graph factory](/oss/python/langchain/mcp/connections#long-lived-connections), read the user's identity and build the MCP client with a per-user token, so the connection carries that user's authorization.
