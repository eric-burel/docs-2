...
[`MCPAdapter`](https://reference.langchain.com/python/langchain/mcp/adapter/MCPAdapter) manages an MCP connection for you, but how long that connection lives depends on how you use it. This page covers the connection lifecycle, pooling connections across a fleet, running against several servers at once, the two MCP protocol eras, and response caching. The connection itself is FastMCP's; each section links out to the relevant FastMCP client documentation.
...
A deployment that serves many runs should discover per run but reuse its connections underneath, rather than reconnecting on every request. In a [`langgraph dev`](/oss/python/langgraph/local-server) graph factory, build the agent inside the factory function so each run picks up the current tool catalog, and let a [shared connection pool](#shared-connection-pool) and [response cache](#caching) absorb the cost:
...
Use `ClientGroup` when servers need independent connections: different [protocol eras](#protocol-eras), [per-server authentication](/oss/python/langchain/mcp/auth#per-server-authentication), or a [shared pool](#shared-connection-pool) configured per client. Use an `MCPConfig` dict when a single aggregate connection is enough.
...
- [Deploy a LangGraph server](/oss/python/langgraph/local-server)
