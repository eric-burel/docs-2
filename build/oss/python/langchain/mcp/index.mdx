...
[`MCPAdapter`](https://reference.langchain.com/python/langchain/mcp/adapter/MCPAdapter) is built on [FastMCP](https://gofastmcp.com), which handles transport inference, protocol negotiation, connection management, and authentication. This section covers the LangChain-specific layer; it links out to the FastMCP client documentation for the connection details underneath.
...
Open an [`MCPAdapter`](https://reference.langchain.com/python/langchain/mcp/adapter/MCPAdapter) as an async context manager, discover the server's tools with `list_tools()`, and hand them to an agent. The tools hold the client, so they stay callable after the context exits:

<McpQuickstartPy />

Pass the resulting tools to [`create_agent`](https://reference.langchain.com/python/langchain/agents/factory/create_agent):
...
<CardGroup cols={2}>
  ...
  <Card title="Elicitation" icon="messages" href="/oss/python/langchain/mcp/elicitation">
    Answer a server's mid-call questions with a human in the loop.
  </Card>
  <Card title="Tool results" icon="arrow-back-up" href="/oss/python/langchain/mcp/tool-results">
...
- [FastMCP client documentation](https://gofastmcp.com/clients/client) — the client [`MCPAdapter`](https://reference.langchain.com/python/langchain/mcp/adapter/MCPAdapter) is built on
...
- [Migrate from `langchain-mcp-adapters`](/oss/python/migrate/langchain-mcp-adapters)
